...
The result with graphs and conclusion will be displayed in the last part. The following is the specification of platform use to process and analyse data.
...
As there are close to a million observations in the raw data and it is important that we do not waste any unnecessary resources to process this data.Therefore we will need to Prepare only the relevant data to support our answers on the questions on health and economic impact.

```{r, PreparedData, cache = TRUE}
healthData <- rawData[(rawData$FATALITIES >0) | (rawData$INJURIES > 0),] 
kable(dim(healthData))

economicData <- rawData[(rawData$CROPDMG > 0) | (rawData$PROPDMG > 0),]
dim(economicData)
```
The relevant health data contains 'r dim(healthData)'
